...
export class UserApi extends runtime.BaseAPI {
    ...
    /**
     * * Mouse button settings are contained in `reverseScrollWheelZoomDirection` and `viewManipulationMouseKeyMapping`.  * For each action in `viewManipulationMouseKeyMapping`, an array of modifier key/mouse combos is provided that performs that action.  * Possible modifier keys include SHIFT and CTRL.  * Possible mouse buttons include MMB (middle), RMB (right), and SCROLLWHEEL.  * The scroll wheel is used in zoom operations, where scrolling forward causes the view to zoom in unless `reverseScrollWheelZoomDirection` is set to true.
