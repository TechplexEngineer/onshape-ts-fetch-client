/* tslint:disable */
/* eslint-disable */
/**
 * Onshape REST API
 * The Onshape REST API consumed by all client. # Authorization The simplest way to authorize and enable the **Try it out** functionality is to sign in to Onshape and use the current session. The **Authorize** button enables other authorization techniques. To ensure the current session isn\'t used when trying other authentication techniques, make sure to remove the Onshape cookie as per the instructions for your particular browser. Alternatively, a private or incognito window may be used. Here\'s [how to remove a specific cookie on Chrome](https://support.google.com/chrome/answer/95647#zippy=%2Cdelete-cookies-from-a-site). - **Current Session** authorization is enabled by default if the browser is already signed in to [Onshape](/). - **OAuth2** authorization uses an Onshape OAuth2 app created on the [Onshape Developer Portal](https://dev-portal.onshape.com/oauthApps). The redirect URL field should include `https://cad.onshape.com/glassworks/explorer/oauth2-redirect.html`. - **API Key** authorization using basic authentication is also available. The keys can be generated in the [Onshape Developer Portal](https://dev-portal.onshape.com/keys). In the authentication dialog, enter the access key in the `Username` field, and enter the secret key in the `Password` field. Basic authentication should only be used during the development process since sharing API Keys provides the same level of access as a username and password.
 *
 * The version of the OpenAPI document: 1.168.20544-ab34447a30b0
 * Contact: api-support@onshape.zendesk.com
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */

import { exists, mapValues } from '../runtime';
/**
 * 
 * @export
 * @interface BTFeatureScriptEvalCall2377
 */
export interface BTFeatureScriptEvalCall2377 {
    /**
     * 
     * @type {string}
     * @memberof BTFeatureScriptEvalCall2377
     */
    btType?: string;
    /**
     * 
     * @type {number}
     * @memberof BTFeatureScriptEvalCall2377
     */
    libraryVersion?: number;
    /**
     * 
     * @type {boolean}
     * @memberof BTFeatureScriptEvalCall2377
     */
    microversionSkew?: boolean;
    /**
     * 
     * @type {{ [key: string]: Array<string>; }}
     * @memberof BTFeatureScriptEvalCall2377
     */
    queries?: { [key: string]: Array<string>; };
    /**
     * 
     * @type {boolean}
     * @memberof BTFeatureScriptEvalCall2377
     */
    rejectMicroversionSkew?: boolean;
    /**
     * 
     * @type {string}
     * @memberof BTFeatureScriptEvalCall2377
     */
    script?: string;
    /**
     * 
     * @type {string}
     * @memberof BTFeatureScriptEvalCall2377
     */
    serializationVersion?: string;
    /**
     * 
     * @type {string}
     * @memberof BTFeatureScriptEvalCall2377
     */
    sourceMicroversion?: string;
}

/**
 * Check if a given object implements the BTFeatureScriptEvalCall2377 interface.
 */
export function instanceOfBTFeatureScriptEvalCall2377(value: object): boolean {
    let isInstance = true;

    return isInstance;
}

export function BTFeatureScriptEvalCall2377FromJSON(json: any): BTFeatureScriptEvalCall2377 {
    return BTFeatureScriptEvalCall2377FromJSONTyped(json, false);
}

export function BTFeatureScriptEvalCall2377FromJSONTyped(json: any, ignoreDiscriminator: boolean): BTFeatureScriptEvalCall2377 {
    if ((json === undefined) || (json === null)) {
        return json;
    }
    return {
        
        'btType': !exists(json, 'btType') ? undefined : json['btType'],
        'libraryVersion': !exists(json, 'libraryVersion') ? undefined : json['libraryVersion'],
        'microversionSkew': !exists(json, 'microversionSkew') ? undefined : json['microversionSkew'],
        'queries': !exists(json, 'queries') ? undefined : json['queries'],
        'rejectMicroversionSkew': !exists(json, 'rejectMicroversionSkew') ? undefined : json['rejectMicroversionSkew'],
        'script': !exists(json, 'script') ? undefined : json['script'],
        'serializationVersion': !exists(json, 'serializationVersion') ? undefined : json['serializationVersion'],
        'sourceMicroversion': !exists(json, 'sourceMicroversion') ? undefined : json['sourceMicroversion'],
    };
}

export function BTFeatureScriptEvalCall2377ToJSON(value?: BTFeatureScriptEvalCall2377 | null): any {
    if (value === undefined) {
        return undefined;
    }
    if (value === null) {
        return null;
    }
    return {
        
        'btType': value.btType,
        'libraryVersion': value.libraryVersion,
        'microversionSkew': value.microversionSkew,
        'queries': value.queries,
        'rejectMicroversionSkew': value.rejectMicroversionSkew,
        'script': value.script,
        'serializationVersion': value.serializationVersion,
        'sourceMicroversion': value.sourceMicroversion,
    };
}

